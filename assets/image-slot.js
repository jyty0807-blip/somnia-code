...
(() => {
  ...
  // on SVG blobs is inconsistent). GIF IS accepted but stored as raw bytes
  // (no canvas re-encode) so animated GIFs keep every frame.
  const ACCEPT = ['image/png', 'image/jpeg', 'image/webp', 'image/avif', 'image/gif'];
  // Formats we must NOT push through the canvas re-encode (it flattens them to
  // a single still frame). Stored verbatim as a data URL instead.
  const RAW_TYPES = ['image/gif'];
  // Guard rail so a huge animated GIF doesn't bloat the sidecar past the host
  // write limit. ~12MB of raw bytes (~16MB base64).
  const RAW_MAX_BYTES = 12 * 1024 * 1024;
  ...
  // Read a file's raw bytes as a data URL, untouched. Used for animated
  // formats (GIF) that the canvas path would flatten to one frame.
  ...
  const stylesheet =
    ...
    '.ctl{position:absolute;top:8px;right:8px;left:auto;transform:none;' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '</div>' +
        '<div class="ctl"><button data-act="replace" title="이미지 교체">교체</button>' +
        '  <button data-act="clear" title="이미지 삭제">삭제</button></div>' +
    ...
    async _ingest(file) {
      ...
      if (RAW_TYPES.indexOf(file.type) >= 0 && file.size > RAW_MAX_BYTES) {
        this._setError('That GIF is too large (max 12MB).');
        return;
      }
      ...
      try {
        ...
        const url = RAW_TYPES.indexOf(file.type) >= 0
          ? await fileToDataUrl(file)
          : await toDataUrl(file, w);
