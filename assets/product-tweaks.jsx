/* SOMNIA PDP — Tweaks (font scale + accent + sticky bar) */
...
function ProductTweaks(){
  const [t, setTweak] = useTweaks(TWEAK_DEFAULTS);

  React.useEffect(()=>{
    document.documentElement.style.fontSize = (t.fontScale/100*100).toFixed(2) + '%';
  },[t.fontScale]);
  ...
  React.useEffect(()=>{
    document.documentElement.style.setProperty('--shot-radius', t.imageRadius + 'px');
  ...
  return (
    <TweaksPanel>
      ...
      <TweakColor label="강조색" value={t.accent}
                  options={['#3a4f8a','#7a5a93','#b89a5e','#195B82']}
...
(function mount(){
  ...
  if(root && window.ReactDOM && window.useTweaks){
    ReactDOM.createRoot(root).render(<ProductTweaks/>);
  } else {
    setTimeout(mount, 80);
  }
})();
