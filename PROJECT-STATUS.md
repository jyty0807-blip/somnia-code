...
- [x] 긴급 수정 (모달 배경 흰색 고정, 배송지 우편번호 Daum Postcode API 연동, 관리자 재고·번들 수정)
- [x] 기획세트 앱 연동 (getBundles, 번들 목록·상세·구매, 단품 재고 차감 방식)
- [x] 주문 상세화 + 멤버십 적립금 (결제 수단 선택, 쿠폰, 예상 적립금, 주문 완료 화면)
...
### Phase 7b: 긴급 수정 ✅
- [x] 관리자 사이트 재고 조회 불가 (Firestore Rules 설정 + 재고 시드)
- [x] 관리자 사이트 번들 미표시 (데이터 미등록 상태, 관리자에서 추가)
- [x] 앱 장바구니 확인 모달 배경 투명 → 불투명 흰색으로 변경
- [x] 앱 배송지 추가 기능 수정 + 우편번호 조회 기능 추가 (Daum Postcode API)

### Phase 8a: 기획세트 앱 연동 ✅
- [x] firebase.js에 getBundles() 추가 (Firestore bundles 컬렉션 조회)
- [x] TabShop 번들 섹션 (목록 카드, 품절 표시)
- [x] ScreenBundleDetail 상세 화면 (구성 상품, 세트 할인, 장바구니 담기)
- [x] ScreenCart 번들 아이템 처리 (장바구니 표시, 주문 시 단품 재고 차감)
- [x] app.jsx 번들 로딩·라우팅·장바구니 로직
- [x] i18n 번들 키 추가 (bundle_section, bundle_items, bundle_orig, bundle_save)

### Phase 8b: 주문 상세화 + 멤버십 적립금 ✅
- [x] ScreenCheckout 주문·결제 화면 (결제 수단 선택, 쿠폰 사용, 예상 적립금)
- [x] ScreenOrderComplete 주문 완료 화면 (결제 금액, 적립 포인트)
- [x] 등급별 적립률 (Cloud 1%, Pastel 3%, Lavender 5%, Midnight 8%)
- [x] 쿠폰 할인 (10% / ₩5,000 / 무료배송)
- [x] 장바구니 → 결제 → 완료 3단계 플로우
